...
# === API CALL (POST met params, ZONDER [] in keys – exact als je andere calcs) ===
...
if st.button("Analyseer", type="secondary"):
    with st.spinner("Calculating hidden location potential..."):
        ...
        # Target CSm²I (1.0 of benchmark)
        if use_benchmark and benchmark_store_id in df["shop_id"].values:
            benchmark_row = df.loc[df["shop_id"] == benchmark_store_id]
            target_csm2i = float((benchmark_row["actual_spsqm"] / (benchmark_row["expected_spsqm"] + eps)).iloc[0])
            target_label = f"benchmark: {benchmark_store_name} (CSm²I {target_csm2i:.2f})"
        else:
            target_csm2i = 1.0
            target_label = "CSm²I = 1.0"

        # Uplift t.o.v. target
        df["uplift_eur"] = np.maximum(0.0, (target_csm2i * df["expected_spsqm"] - df["actual_spsqm"])) * sq

        # === KPI‑BANNER (duidelijk over periode & target) ===
        ...
        st.markdown(f"""
            <div style='background-color: #FEAC76; color: #000000;
                ...
                🚀 Potential revenue growth <span style='opacity:.75'>(over <b>{period}</b>, target {target_label})</span><br/>
        ...
        st.markdown(
            ...
            **Toelichting**  
            ...
            - **Uplift (€)** = *(target_CSm²I × expected − actual) × m²*, dus extra omzet in **{period}**  
              als de winkel naar **target {target_label}** groeit (zonder extra traffic).  
            """
        )
